...
SYMBOL = 'BTC/USDT'
TIMEFRAME = '15m' 
MODEL_PATH = "models/sniper_pro_gen_6.zip" # Este é o seu cérebro atual
...
wins = 0
losses = 0
DAILY_LOSS_LIMIT = -0.05  # -5% de limite de perda diária
...
state = {
    ...
    "order_book": [], 
    "adaptation": {
        "generation": 1, # Resetando para Geração 1 (Marco Zero)
...
def load_brain(path=MODEL_PATH):
    ...
    try:
        if os.path.exists(path):
            ...
            print(f">>> ⚠️ AVISO: Modelo {path} não encontrado no HD do servidor.")
        ...
        try:
            ...
            if closed_candle_ts > last_saved_candle_ts:
                ...
                if os.path.exists(DATA_PATH):
                    df_to_save.to_csv(DATA_PATH, mode='a', header=False, index=False)
                else:
            ...
            if model and len(df_clean) > 0:
                if startup_phase:
                    ...
                    is_choppy = current_atr_pct < 0.0015 
                    ...
                    if warming_up:
                        ...
                        if kill_switch_active:
                            target_pos = 0
                            state["status"] = "🛑 KILL-SWITCH: LIMITE DE PERDA ATINGIDO."
                        elif act_idx == 0:
                            target_pos = 0
                        elif is_choppy and position == 0:
                            target_pos = 0 
                            state["status"] = "💤 MERCADO LATERAL (AGUARDANDO)"
                        elif consecutive_signals >= 2:
                            if act_idx == 1: # LONG
                                if macro_trend == 1: target_pos = 1
                                else: state["status"] = "🛡️ FILTRO MACRO: BLOQUEANDO LONG"
                            elif act_idx == 2: # SHORT
                                if macro_trend == -1: target_pos = -1
                                else: state["status"] = "🛡️ FILTRO MACRO: BLOQUEANDO SHORT"
                            consecutive_signals = 0 
                        else:
                    ...
                    if not warming_up and position != 0:
                        ...
                        if unrealized_pct <= dynamic_stop:
                            target_pos = 0  
                            state["order_book"].insert(0, {"text": f"[{datetime.now().strftime('%H:%M:%S')}] 🛡️ STOP ACIONADO ({unrealized_pct*100:.2f}%)"})
                    ...
                    if target_pos != position:
                        if position != 0: # FECHAMENTO
                            ...
                            cor = "🟢" if pnl >= 0 else "🔴"
                            state["order_book"].insert(0, {"text": f"[{horario}] {cor} FECHOU | PnL: ${pnl:.2f}"})
            ...
            for _, row in last_100.iterrows():
                clean_history.append({
                    ...
                    "rsi": float(row['rsi']), "bb_width": float(row['bb_width']) 
                })
...
async def download_dados(senha: str):
    ...
    if not os.path.exists(DATA_PATH): raise HTTPException(status_code=404, detail="Sem dados.")
...
async def upload_cerebro(senha: str, file: UploadFile = File(...)):
    ...
    state["order_book"].insert(0, {"text": f"[{datetime.now().strftime('%H:%M:%S')}] 🧬 EVOLUÇÃO: Geração {new_gen}..."})
    ...
    if os.path.exists(DATA_PATH): os.rename(DATA_PATH, f"data/archive_reboot_gen_{current_gen}.csv")
